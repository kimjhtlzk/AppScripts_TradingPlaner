...
function applyFormulas(sheet, row) {
  // B열: 종목명/지수명
  var nameFormula = '=IF(A' + row + '="","",IF(E' + row + '="KR_INDEX",A' + row + ',IF(E' + row + '="INDEX",IFERROR(GOOGLEFINANCE(A' + row + ',"name"),A' + row + '),IF(E' + row + '="NASDAQ",IFERROR(GOOGLEFINANCE(A' + row + ',"name"),A' + row + '),IFERROR(REGEXEXTRACT(IMPORTXML("https://finance.naver.com/item/main.nhn?code="&A' + row + ',"//title"),"^([^:]+)"),"")))))';

  // C열: 현재가/지수 레벨
  var priceFormula = '=IF(A' + row + '="","",IF(E' + row + '="KR_INDEX",IFERROR(VALUE(SUBSTITUTE(SUBSTITUTE(IMPORTXML("https://finance.naver.com/sise/sise_index.naver?code="&A' + row + ',"//*[@id=\'now_value\']"),",","")," ","")),"N/A"),IF(E' + row + '="INDEX",IFERROR(GOOGLEFINANCE(A' + row + ',"price"),"N/A"),IF(E' + row + '="NASDAQ",IFERROR(GOOGLEFINANCE(A' + row + ',"price"),"N/A"),IFERROR(VALUE(SUBSTITUTE(IMPORTXML("https://finance.naver.com/item/sise.naver?code="&A' + row + ',"//strong[@id=\'_nowVal\']"),",","")),"로딩 실패")))))';

  // D열: 등락률 (커스텀 함수 사용)
  var changeFormula = '=IF(A' + row + '="","",IF(E' + row + '="KR_INDEX",getKRIndexChangeRate(A' + row + '),IF(E' + row + '="INDEX",IFERROR((GOOGLEFINANCE(A' + row + ',"price")-GOOGLEFINANCE(A' + row + ',"closeyest"))/GOOGLEFINANCE(A' + row + ',"closeyest"),"N/A"),IF(E' + row + '="NASDAQ",IFERROR(GOOGLEFINANCE(A' + row + ',"changepct")/100,"N/A"),IFERROR(VALUE(SUBSTITUTE(SUBSTITUTE(IMPORTXML("https://finance.naver.com/item/sise.naver?code="&A' + row + ',"//strong[@id=\'_rate\']//span"),"%",""),",",""))/100,"로딩 실패")))))';
  ...
  if (market === "KR_INDEX" || market === "INDEX") {
    // 지수: 숫자만 표시, 소수점 2자리
